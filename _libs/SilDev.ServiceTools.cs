...
namespace SilDev
{
    public class ServiceTools
    {
        ...
        public static void InstallService(string _serviceName, string _displayName, string _path, string _args)
        {
            ...
            try
            {
                IntPtr service = OpenService(scman, _serviceName, ServiceRights.QueryStatus | ServiceRights.Start);
                if (service == IntPtr.Zero)
                    service = CreateService(scman, _serviceName, _displayName, ServiceRights.QueryStatus | ServiceRights.Start, SERVICE_WIN32_OWN_PROCESS, ServiceBootFlag.AutoStart, ServiceError.Normal, string.Format("{0} {1}", _path, _args).TrimEnd(), null, IntPtr.Zero, null, null, null);
                if (service == IntPtr.Zero)
        ...
        public static void UninstallService(string _name)
        {
            ...
            try
            {
                IntPtr service = OpenService(scman, _name, ServiceRights.StandardRightsRequired | ServiceRights.Stop | ServiceRights.QueryStatus);
                if (service == IntPtr.Zero)
        ...
        public static bool ServiceExists(string _name)
        {
            ...
            try
            {
                IntPtr service = OpenService(scman, _name, ServiceRights.QueryStatus);
                if (service == IntPtr.Zero)
                    return false;
                CloseServiceHandle(service);
                return true;
        ...
        public static void StartService(string _name)
        {
            ...
            try
            {
                IntPtr hService = OpenService(scman, _name, ServiceRights.QueryStatus |
                ServiceRights.Start);
        ...
        {
            IntPtr scman = OpenSCManager(null, null, _rights);
