...
namespace Updater
{
    public static class Lang
    {
        ...
        public static string GetText(string _lang, Control _obj)
        {
            ...
            {
                case "de-DE":
                    ResManager = new ResourceManager(string.Format("Updater.LangResources.{0}", _lang), Assembly.Load(Assembly.GetEntryAssembly().GetName().Name));
                    text = ResManager.GetString(_obj.Name);
                    break;
                default:
                    ResManager = new ResourceManager("Updater.LangResources.en-US", Assembly.Load(Assembly.GetEntryAssembly().GetName().Name));
                    text = ResManager.GetString(_obj.Name);
                    break;
            }
